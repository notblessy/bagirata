      ...
      <React.Fragment key={f.id}>
        <Grid pb={35}>
          <Grid.Col span={12} pb={0}>
            <Group grow>
              <Text fz={18} fw={600} c="#161617">{f.name}</Text>
              <Text fz={18} fw={600} c="#161617" ta="right">{`Rp ${grandTotal.toLocaleString()}`}</Text>
            </Group>
          </Grid.Col>
          {
            f?.items?.map((i) => {
              ...
              return (
                <React.Fragment key={i.billID + f.id}>
                  <Grid.Col span={6} m="auto 0" py={0}>
                    <Text fz={14} fw={400} c="dimmed">{i.billName}</Text>
                  </Grid.Col>
                  <Grid.Col span={1} m="auto 0" py={0}>
                    <Text fz={14} fw={400} c="dimmed">{`x${i.qty}`}</Text>
                  </Grid.Col>
                  <Grid.Col span={5} m="auto 0" py={0}>
                    <Text fz={14} fw={400} c="dimmed" ta="right">{`Rp ${i.subTotal.toLocaleString()}`}</Text>
                  </Grid.Col>
                </React.Fragment>
              )
            })
      ...
      <Box style={{ padding: '10px 20px' }}>
        ...
        <Box>
          ...
          <UnstyledButton
            ...
            onClick={() => {
              navigator.clipboard.writeText(user?.bank.bankNumber)
                .then(() => {
                  setCopiedBank(true);
                  notifications.show({
                    title: "Success",
          ...
          >
            <Text fz={12} fw={800}>{user?.bank.bankAccountName}</Text>
            <Group gap={5}>
              <Text fz={12}>{`${user?.bank.bank} - ${user?.bank.bankNumber}`}</Text>
              <IconCopy size={15} />
            </Group>
          </UnstyledButton>
        </Box>
